#!/bin/bash
# Script to launch N PX4 drones in Gazebo Harmonic SITL with explicit Gazebo readiness check.
set -e
echo "LAUNCH SWARM CALLED WITH ARGS: MAP_ID=$1 DRONE_COUNT=$2" >> /tmp/launch_swarm_debug.log
...
# Launch PX4 instances sequentially
for ((i=0; i<$DRONE_COUNT; i++)); do
    ...
    echo "Waiting 15 seconds for Drone $i to initialize..."
    sleep 15
done
